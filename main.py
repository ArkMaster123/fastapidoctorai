...
async def upsert(request: str):
    ...
    try:
        ...
        notion_id = request_data.get('notion_id')
        doc_type = request_data.get('doc_type')
        cleanup_mode = CleanupMode(request_data.get('cleanup_mode', CleanupMode.INCREMENTAL))
        last_update_time = request_data.get('last_update_time')
